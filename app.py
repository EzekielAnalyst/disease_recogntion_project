...
# Load the trained model
path_to_model = current_dir / 'decision_tree_model.pkl'
with path_to_model.open('rb') as file:
    clf = pickle.load(file)

# Load the label encoder
training_path = current_dir / 'Training.csv'
training = pd.read_csv(training_path)
y = training['prognosis']
le = preprocessing.LabelEncoder()
le.fit(y)
...
def getSeverityDict():
    severityDictionary = {}
    file_path = current_dir / 'symptom_severity.csv'
...
def getDescription():
    description_list = {}
    file_path = current_dir / 'symptom_Description.csv'
    if not file_path.exists():
        print(f"File not found: {file_path}")
        return description_list
    with file_path.open() as csv_file:
...
def getprecautionDict():
    precautionDictionary = {}
    file_path = current_dir / 'symptom_precaution.csv'
    if not file_path.exists():
        print(f"File not found: {file_path}")
        return precautionDictionary
    with file_path.open() as csv_file:
